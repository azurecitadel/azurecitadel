...
When you are onboarding VMs at scale then it makes sense to prepare the target environment first. The customer, Wide World Importers, has requested that Azure landing zone (ALZ) is used as they are keen to be aligned to the Microsoft best practice recommendations and leverage the IP baked into the official repos. This will be the first step in the target environment preparation.
...
This is a small POC and you have only been provided with a single subscription but it is still possible to apply ALZ. The expectation is that multiple subscriptions would be used in production as per the [Azure landing zone](https://aka.ms/adopt/ready) documentation.

There are three recommended Microsoft repos for Azure landing zone:
...
Wide World Importers would like a vanilla deployment of Azure landing zone applied to the POC environment to understand how that impacts the virtual machine onboarding.
...
The Azure landing zone creates a useful set of default platform resources, policies and RBAC role assignments. There are a number of custom and built-in policies that are applicable to Azure Arc-enabled Servers. Some are already assigned, and some are unassigned.
